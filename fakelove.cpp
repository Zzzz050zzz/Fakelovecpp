...
void stickman_chat(int speed, int box_color, int text_color)
{
	...
	outtextxy(-20 + s, 210, (char*)"I Love");
	outtextxy(-5 + s, 240, (char*)"You");
...
void stickwomen_chat(int speed, int box_color, int text_color)
{
	...
	setcolor(text_color);
	settextstyle(EUROPEAN_FONT, HORIZ_DIR, 1);
	outtextxy(45 + s, 210, (char*)"Sorry, I love");
	outtextxy(100 + s, 240, (char*)"............");
...
void rain(int speed, int drop_size)
{
	...
	for (int i = 0; i < 30; i++)
	{
		...
		outtextxy(speed + rand() % 150, 100 + rand() % 345, (char*)"");
	}
	setcolor(WHITE);
}
...
void subscribe()
{
	...
	outtextxy(50, 150, (char*)"like our page and channel");
	settextstyle(EUROPEAN_FONT, HORIZ_DIR, 4);
	setcolor(LIGHTBLUE);
	outtextxy(100, 230, (char*)"Cool Programming Projects");
}
int main()
{

	initwindow(700, 550, "LOVEFAKE", 150, 50); 
	int page = 0;
	int sm = 4;
	int sw = 4;
	int n = 0;
